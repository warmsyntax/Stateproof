...
- Dependency budget: each package lists deps explicitly; adding any new runtime dependency requires a one-line justification in the PR body (bundle and install time are product features for a CLI). The Version 1 runtime dependency set is governed by contract §11.7: `commander`, `zod`, `picocolors`, `playwright`, `picomatch`, `jsonc-parser`, `@clack/prompts`, `pixelmatch`, `pngjs`, `@modelcontextprotocol/sdk` — anything else needs a contract change.
...
| Runner integration | Vitest + Playwright against `examples/react-vite-demo` | delay, fixture, error, offline modes each have a passing + failing fixture; visual diff pass & fail |
| CLI contract | Snapshot of stdout JSON envelope + exit codes for: valid run, failed scenario, bad flag, missing file, studio TTY guard | envelope shape changes require version bump discussion |
| Report | Smoke render + a11y axe scan | zero critical violations; diff evidence renders side-by-side |
...
- Screenshots produced by temporary test runs go to temp dirs; baselines are stored in `stateproof/baselines/`.
...
- Threat-model reviews required for: anything touching the filesystem outside `artifacts/stateproof/` and `stateproof/baselines/`, anything reading env vars, any future server component.
...
- **Filesystem discipline:** Stateproof writes only to `artifacts/stateproof/`, `stateproof/baselines/`, plus the files `init` creates (`stateproof.scenarios.json`, `fixtures/`, `.gitignore` entries).
...
- Native MCP server exposes named tools only (`stateproof_list_scenarios`, `stateproof_run`, `stateproof_inspect_failure`, `stateproof_export_card`) - no generic fetch interception, no shell passthrough.
