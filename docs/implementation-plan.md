...
P0.5 Contract freeze  ▓▓▓▓▓▓▓▓▓▓  docs aligned to contract.md
...
P5 Golden-path proof  ▓▓▓▓▓▓▓▓▓▓  example app + E2E + contract §12 verification
─── Version 1 Expansion (IMPROVED_PRD.md) ──────────────────────────
Phase 1: Contract, Architecture & Core Types  [x]
Phase 2: Visual Baseline Regression & Smart Selector Analyzer [x]
Phase 3: Shared Orchestration Package (@stateproof/app) [x]
Phase 4: Interactive Terminal Studio (stateproof studio) [x]
Phase 5: Native MCP Bridge (@stateproof/mcp-server) [x]
Phase 6: Full Verification & Golden Path End-to-End [x]
```
...
- [x] Browser lifecycle: Chromium launched once per run, headless default; fresh context per scenario×viewport (`serviceWorkers: "block"`, locale `en-US`, timezoneId `UTC`, viewport/dsf/isMobile from config; `ignoreHTTPSErrors` only for HTTPS loopback)
- [x] **One catch-all route handler**, registered before goto; every request resolved (fulfill / abort / `route.fetch()`); handler errors abort safely; never leave a route pending
- [x] Navigation URL = `new URL(route, baseUrl)`; `waitUntil: "domcontentloaded"` (never `load`)
- [x] Request classification: `data:`/`blob:` pass; third-party origin ⇒ abort `"blockedbyclient"` unless `--allow-third-party` (`route.fetch()`); matched method+pathname ⇒ response rule; else passthrough. Query strings ignored; method exact match
- [x] `delay`: hold first matching request pending during capture, then abort `"aborted"`; later matches abort immediately; no interception before timeout ⇒ `request-not-intercepted`. **No real-response passthrough**
- [x] `offline`: abort matched requests `"internetdisconnected"` at request level — never `context.setOffline(true)`
- [x] `fixture`/`inline`/`error`: fulfill with correct status/headers/Content-Type defaults per contract §6.4
- [x] Selector waiting: all selectors visible (array support); `stableMs` continuous-visibility wait with restart on hide; scroll first selector into view; **viewport-only screenshot** (never fullPage)
- [x] Failure capture: failure screenshot with same semantic filename; exact failureCodes (`selector-timeout`, `selector-unstable`, `navigation-failed`, `capture-failed`); unexpected crash ⇒ status `error` + `trace.md` written if possible
- [x] Artifact lockfile `.lock` acquisition (`ARTIFACT_LOCKED` exit 2 if present); stale generated-file cleanup before run; lock removed in `finally`
- [x] `runId` generated once per run; global scenario watchdog so repeated runs never hang; zero unresolved routes across the suite
- [x] Integration tests vs `examples/react-vite-demo` covering contract §10.2: delay/fixture/inline/error/offline pass+fail cases, selector-timeout failure screenshot, non-matching method passthrough, third-party blocked by default, delayed request aborted after capture

**Exit gate:** contract §10.2 list green; zero hangs across 50 consecutive runs. ✅ Verified against contract §10.2 test matrix.

## P3 — CLI (`@stateproof/cli`)

**Goal:** The exact flag grammar and agent contract from contract §5.

- [x] Commander wiring: `init` · `run` · `list` · `export` with **only** the contract flags (`--file`, `--url`, `--route`, `--force`, `--scenario`, `--viewport`, `--timeout-ms`, `--allow-remote`, `--allow-third-party`, `--strict-secrets`, `--reporter`, `--run`, `--format`)
- [x] Positional resolution rules implemented verbatim (contract §5.2); unknown scenario ids / zero selected runs ⇒ exit 2
- [x] Precedence: `--url` > file `baseUrl`; `expect.timeoutMs` > `--timeout-ms` > 15000
- [x] `init`: no reachability check, no browser launch; refuses overwrite without `--force`; scaffolds **strict JSON with `$comment` and `note` fields only**; creates fixtures dir + artifacts gitignore entry
- [x] `run`: reachability check (10 s budget ⇒ exit 3 `APP_UNREACHABLE`); loopback guardrail incl. DNS resolution of hostnames; redirect guardrail (`NON_LOOPBACK_REDIRECT`)
- [x] stdout/stderr discipline: `--reporter json` = one JSON document on stdout; human reporter styled output allowed on stdout; diagnostics always stderr; NO_COLOR respected; no prompts; no spinners when piped
- [x] Envelope types `init.result`/`list.result`/`run.result`/`export.card`/`error`; `ok: true` only on exit 0; partial results + error on fatal mid-run
- [x] Exact machine error codes table (contract §7.5) wired to exits; every hint names the next action
- [x] Human terminal output per design.md §5.1 (picocolors); statuses printed as words PASS/FAIL/ERROR
- [x] `export`: `--run` resolution chain per contract (explicit → file name → single-dir fallback → exit 2 asking for `--run`)
- [x] CLI contract tests spawn real processes; snapshot normalization replaces timestamps, durations, runId, browser version, absolute paths (contract §10.3)
...
- [x] **Report is a directory**: `report/index.html` + screenshots copied into `report/assets/*.png`
- [x] Works from `file://`; zero network requests; inline CSS only; **no inline JavaScript**; no external fonts/CSS/images
- [x] **All dynamic strings HTML-escaped** (ids, labels, notes, selectors, URLs, messages, hints, metadata); no raw innerHTML
- [x] State matrix table (scenario × viewport, word + glyph), screenshot gallery, failure details block
- [x] Footer metadata: Stateproof version, browser version, runId, ISO timestamp, scenario file name
- [x] Statuses limited to PASS / FAIL / ERROR (no WARN/SKIP anywhere in v0.1 output)
- [x] A11y: axe scan zero critical violations, keyboard navigable, proper `<th scope>`, alt text per screenshot
- [x] Report tests per contract §10.4 (file:// render, assets exist, no external URLs, escaping verified)

**Exit gate:** report opens from `file://` with networking disabled; design checklist §8 all checked. ✅ Verified against contract §10.4 test matrix.

## P5 — Golden-path proof → **v0.1**

**Goal:** Contract §12 definition of done demonstrated, not assumed.

- [x] Flesh out `examples/react-vite-demo`: settings screen with deterministic state branches, **no external network, no external fonts**; stable selectors `[data-state='loading']`, `[data-state='empty']`, `[data-state='error']`, `[data-state='offline']`, `[data-testid='retry']`; mobile bug toggle hides retry control under 420px; loading proven via delay mode; offline proven via request-level abort
- [x] Scripted demo: setup → `init` → scenarios × viewports → intentional failure → fix → rerun green → paste card
- [x] Full E2E in CI runs the scripted demo headlessly on Ubuntu + Windows
- [x] Performance budgets verified: start ≤ 10 s, default timeout 15 s (NFR §9)
- [x] Docs sweep: READMEs per package, planning docs updated to "as-built" state
- [x] Tag `v0.1.0`

**Exit gate:** every checkbox in contract §12 true (exit codes verified, JSON envelope verified, report opens offline, screenshots exist, card exports correctly, security rules enforced, no external network required by tests, CI green both OS). ✅ Ready for v0.1.0 release.
