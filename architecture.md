...
stateproof/
├── apps/
│   └── web/                        # Marketing + docs site (later; M0 demo lives here first)
├── packages/
│   ├── core/                       # @stateproof/core        — schema, matcher, result model, card renderer
│   ├── playwright-runner/          # @stateproof/playwright-runner — browser execution engine
│   ├── reporter-html/              # @stateproof/reporter-html     — offline HTML report generator
│   ├── cli/                        # @stateproof/cli (bin: stateproof)
│   └── github-action/              # @stateproof/github-action     — JS action wrapper (R2)
├── examples/
...
```
cli ──────────► core ◄────────── reporter-html
 │                                   ▲
 ├──► playwright-runner ─────────────┘ (runner produces RunResult from core)
 └──► reporter-html
github-action ──► cli (invokes npx stateproof run --reporter json)
core ◄── playwright-runner (imports types/schema only)
```

Nothing may depend on `cli`. The GitHub Action shells out to the CLI binary — it must never import runner internals.
